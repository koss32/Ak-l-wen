...
const config={deliveryReady:true,sourceLegalStatus:'published',privacyUrl:'https://example.test/privacy',privacyStatus:'published',consentVersion:legal.consentVersion,staffUserIds:['55','56'],staffChatId:'99'};
...
test('/stop invalidates an earlier opt-in button even when reminders were already off',async()=>{
 const {store,bot,record}=await fixture({confirmed:true});
 await bot.handle(message(1,'/status'));
...
test('ambiguous reminder delivery is not recreated by another opt-in command',async()=>{
 const {store,bot}=await fixture({confirmed:true});
...
test('staff prompts work with Telegram group privacy and previews can be edited safely',async()=>{
 const {store,bot,record}=await fixture();
 await bot.handle(message(1,`/staff ${record.id}`,99,55,'group'));
 await bot.handle(callback(2,await action(store,'staff-reply'),99,55,'group'));
 assert.equal((await recent(store,99)).meta.reply_markup.force_reply,true);
 await bot.handle(message(3,'First draft',99,55,'group'));
 const first=await action(store,'staff-reply-commit');
 await bot.handle(message(4,'Revised draft',99,55,'group'));
 const revised=await action(store,'staff-reply-commit');
 await bot.handle(callback(5,first,99,55,'group'));
 assert.match((await recent(store,99)).text,/veraltet/);
 await bot.handle(callback(6,revised,99,56,'group'));
 assert.ok((await store.inspect()).actions[revised.slice(2)]);
 await bot.handle(callback(7,revised,99,55,'group'));
