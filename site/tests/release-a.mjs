...
try{
 ...
 await p.fill('#name','Test Person');await p.selectOption('#schedule-brand','valset');
 ...
 await p.locator('[data-question]').click();assert.ok(await p.locator('#question-dialog').isVisible());assert.equal(await p.locator('#question-dialog a').getAttribute('href'),'https://t.me/ak_loewenbot?start=site_question');
 ...
 await p.keyboard.press('Escape');assert.ok(await p.locator('[data-question]').evaluate(e=>e===document.activeElement));
