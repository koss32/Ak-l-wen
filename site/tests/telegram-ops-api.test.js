...
import {runCheck,runRemoteCheck,PREVIEW_ORIGIN} from '../server/telegram-ops.js';

const SECRET='w'.repeat(40);
const env=()=>({VERCEL:'1',VERCEL_ENV:'preview',VERCEL_GIT_COMMIT_REF:'release-3',PUBLIC_ORIGIN:PREVIEW_ORIGIN,BOT_ENABLED:'true',BOT_WEBHOOK_ENABLED:'true',TELEGRAM_WORKER_SECRET:SECRET});
...
test('remote diagnostics refuse non-Preview deployments and never expose env',async()=>{
 for(const change of [{VERCEL:'0'},{VERCEL_ENV:'production'},{VERCEL_GIT_COMMIT_REF:'release-2'},{PUBLIC_ORIGIN:'https://other.example'},{BOT_ENABLED:'false'}]){
...
test('remote CLI uses only the fixed Preview endpoint and copies a redacted schema',async()=>{
 ...
 const calls=[],checked=await runRemoteCheck({env:{TELEGRAM_WORKER_SECRET:SECRET},fetchImpl:async(url,options)=>{calls.push({url,options});return {status:503,json:async()=>result};}});
 assert.equal(calls.length,1);assert.equal(calls[0].url,`${PREVIEW_ORIGIN}/api/telegram-ops/`);assert.equal(calls[0].options.redirect,'error');assert.equal(calls[0].options.headers.authorization,`Bearer ${SECRET}`);
...
test('remote CLI never prints secret-bearing error bodies or malformed result fields',async()=>{
 ...
 for(const body of [{ok:false,error:SECRET},{...base,errors:[SECRET]},{...base,telegram:{...base.telegram,pending_count:SECRET}},{...base,staff:{...base.staff,mode:SECRET}}]){
  const result=await runRemoteCheck({env:{TELEGRAM_WORKER_SECRET:SECRET},fetchImpl:async()=>({status:503,json:async()=>body})});
