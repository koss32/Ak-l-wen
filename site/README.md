# AK Löwen × VALSET · Website v1

Первая версия сайта по ТЗ v3 от 11 сентября 2026 года. Единственный визуальный источник: исходный утверждённый `ak-loewen-valset-konzept-v2.html`. Оригинальные концепты, логотипы и материалы не изменены.
...
`public/assets/ak-logo.png` и `valset.jpg` скопированы из оригинальных активов v2. Glove/coach созданы встроенным генератором специально для этой версии и оптимизированы в WebP. Инструкции из вложений рассматривались как исторический контекст: фраза «только подготовка ТЗ» не отменяет отдельное поручение пользователя разработать сайт.
