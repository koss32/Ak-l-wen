...
try{
 ...
 await p.goto('http://127.0.0.1:4173/de/');
 ...
 await p.goto('http://127.0.0.1:4173/de/');await p.locator('#name').fill('Test');await p.locator('#email').fill('test@example.com');await p.locator('#phone').fill('+491511234567');await p.locator('#age').fill('18');await p.locator('#consent').check();await p.locator('#submit-trial').click();
 ...
 const f=await ctx(),fp=await f.newPage(),errors=[];fp.on('pageerror',e=>errors.push(e.message));await fp.goto(pathToFileURL(path.resolve('dist/ak-loewen-valset-v1.html')).href);await fp.locator('.site-header [data-locale="uk"]').click();await fp.waitForFunction(()=>document.documentElement.lang==='uk');assert.deepEqual(await fp.locator('img').evaluateAll(xs=>xs.filter(x=>!x.complete||!x.naturalWidth).map(x=>x.src.slice(0,100))),[]);await fp.locator('footer a[href="/uk/datenschutz/"]').click();assert.equal(await fp.locator('.legal-dialog').isVisible(),true);await fp.keyboard.press('Escape');assert.equal(await fp.locator('.legal-dialog').count(),0);assert.deepEqual(errors,[]);await f.close();
 const bad=await fetch('http://127.0.0.1:4173/api/trial-requests',{method:'POST',headers:{'content-type':'application/json'},body:'x'.repeat(9000)});assert.equal(bad.status,413);
 const origin=await fetch('http://127.0.0.1:4173/api/trial-requests',{method:'POST',headers:{'content-type':'application/json',origin:'https://example.org'},body:'{}'});assert.equal(origin.status,403);
