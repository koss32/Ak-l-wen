...
const live=legal.publicationStatus==='published'&&!!process.env.TELEGRAM_BOT_TOKEN&&!!process.env.TELEGRAM_CHAT_ID_AK;
...
const server=http.createServer(async(req,res)=>{
 ...
 if(url.pathname==='/api/trial-requests'){
  ...
  if(req.headers.origin&&req.headers.origin!==allowedOrigin)return json(403,{ok:false,code:'origin'});
