...
- После валидного клика inline-кнопки удаляется устаревшая карточка бота, поэтому предыдущие меню не накапливаются. Сообщения пользователя и важные ответы тренера не удаляются автоматически.
