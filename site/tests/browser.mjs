...
try{
 ...
 await p.locator('#directionId').selectOption('valset');assert.equal(await p.locator('#ak-fields').isVisible(),false);assert.equal(await p.locator('#instagram-flow').isVisible(),true);await p.locator('#groupId').selectOption('val-mama');assert.match(await p.locator('#instagram-message').innerText(),/Mama & Kind/);assert.match(await p.locator('#instagram-flow a').getAttribute('href'),/instagram.com\/VALSET_SOLINGEN/);checks.push('VALSET has separate Instagram flow without personal fields');
 await p.locator('.site-header [data-locale="ru"]').click();await p.waitForFunction(()=>document.documentElement.lang==='ru');assert.equal(await p.locator('#groupId').inputValue(),'val-mama');assert.equal(await p.locator('#ak-fields').isVisible(),false);
 await c.close();
 const no=await context({javaScriptEnabled:false}),np=await no.newPage();await np.goto(base+'/de/');assert.equal(await np.locator('#submit-trial').isDisabled(),true);assert.equal(await np.locator('#trial-form').getAttribute('method'),'post');await np.locator('#detail-boxen summary').click();assert.equal(await np.locator('#detail-boxen').getAttribute('open'),'');checks.push('no-JavaScript reading, details and safe form');await no.close();
